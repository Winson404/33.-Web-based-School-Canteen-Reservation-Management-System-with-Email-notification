...
if (isset($_POST['update_user'])) {
	...
	if(empty($file)) {
		if ($person->update_check_email_exists($user_Id, $email)) {
	        displayErrorMessage("Email already exists!", '../Admin/users_mgmt.php?page='.$user_Id);
	    } else {
		    ...
		    // var_dump($result);
		    if($result) {
		    	displayUpdateMessage($result, "Record has been updated.", '../Admin/users_mgmt.php?page='.$user_Id);
		    } else {
		    	displayErrorMessage("Something went wrong while updating the information.", '../Admin/users_mgmt.php?page='.$user_Id); 
		    }
	    }
	} else {
		if ($person->update_check_email_exists($user_Id, $email)) {
	        ...
	        if ($check == false) {
	            displayErrorMessage("File is not an image.", '../Admin/users_mgmt.php?page='.$user_Id);
	            $uploadOk = 0;
	        } elseif ($_FILES["fileToUpload"]["size"] > 500000) {
	            displayErrorMessage("File must be up to 500KB in size.", '../Admin/users_mgmt.php?page='.$user_Id);
	            $uploadOk = 0;
	        } elseif ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
	            displayErrorMessage("Only JPG, JPEG, PNG & GIF files are allowed.", $page);
	            $uploadOk = 0;
	        } elseif ($uploadOk == 0) {
	            displayErrorMessage("Your file was not uploaded.", '../Admin/users_mgmt.php?page='.$user_Id);
	        } else {
	            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
	            	...
	            	displayErrorMessage("There was an error uploading your profile picture.", '../Admin/users_mgmt.php?page='.$user_Id); 
	            }
	        }
	    }
	}
...
// UPDATE LOGGED IN USER INFROMATION
