	...
	if ($person->check_email_exists($email)) {
        ...
        if ($check == false) {
            ...
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
            	$result = $person->create_user($firstname, $middlename, $lastname, $suffix, $dob, $age, $birthplace, $gender, $civilstatus, $occupation, $religion, $email, $contact, $house_no, $street_name, $purok, $zone, $barangay, $municipality, $province, $region, $HashedPassword, $file);
			    // var_dump($result);
			    if($result) {
...
// CREATE USER - ADMIN/USERS_MGMT.PHP
if (isset($_POST['create_user'])) {
	...
	if ($person->check_email_exists($email)) {
        displayErrorMessage("Email already exists!", '../Admin/users_mgmt.php?page=create');
    } else {
        $target_dir = '../assets/images-users/';
        $target_file = $target_dir . basename($_FILES["fileToUpload"]["name"]);
        $uploadOk = 1;
        ...
        $check = getimagesize($_FILES["fileToUpload"]["tmp_name"]);
        if ($check == false) {
            displayErrorMessage("File is not an image.", '../Admin/users_mgmt.php?page=create');
            $uploadOk = 0;
        } elseif ($_FILES["fileToUpload"]["size"] > 500000) {
            displayErrorMessage("File must be up to 500KB in size.", '../Admin/users_mgmt.php?page=create');
            $uploadOk = 0;
        } elseif ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
            displayErrorMessage("Only JPG, JPEG, PNG & GIF files are allowed.", '../Admin/users_mgmt.php?page=create');
            $uploadOk = 0;
        } elseif ($uploadOk == 0) {
            displayErrorMessage("Your file was not uploaded.", '../Admin/users_mgmt.php?page=create');
        } else {
            if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
            	$result = $person->create_user($firstname, $middlename, $lastname, $suffix, $dob, $age, $birthplace, $gender, $civilstatus, $occupation, $religion, $email, $contact, $house_no, $street_name, $purok, $zone, $barangay, $municipality, $province, $region, $HashedPassword, $file);
			    // var_dump($result);
			    if($result) {
			    	displaySaveMessage($result, '../Admin/users_mgmt.php?page=create');
			    } else {
			    	displayErrorMessage("Something went wrong while saving the information.", '../Admin/users_mgmt.php?page=create'); 
			    }
            } else {
            	displayErrorMessage("There was an error uploading your profile picture.", '../Admin/users_mgmt.php?page=create'); 
            }
        }
    }
